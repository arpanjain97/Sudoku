...
class Grid():
	...
	for row in range(9):
            for col in range(9):
                self.grid_items[row][col]=Entry(self.root,font=('Comic Sans Ms',30,'bold'),fg='red',width=2,disabledforeground='black',borderwidth=5,justify='center',relief='flat')
                self.grid_items[row][col].bind('<FocusOut>',self.__valid)
	self.place(100, 70)
  
    def place(self, originX, originY):
        y_pos = originY
	for row in range(9):
            x_pos=originX
            for col in range(9):
                if col==3 or col==6:
                    x_pos+=10
                self.grid_items[row][col].place(x=65*col+x_pos,y=60*row+y_pos)
            if row==2 or row==5:
                y_pos+=10
...
class Controller(Sudoku,Grid):
    ...
    def Window(self):
        ...
        self.canv.create_image(750,65,image=self.title,anchor='nw',tag='titleBanner')
    def add_widgets(self):
        #adds the required widgets to window
        self.grid = Grid(self.canv)
        self.timer=Timer_app(self.canv)
        self.submit = Button(self.canv,text='SUBMIT',relief='flat',font=(basefont,30,basestyle),fg='white',bg=theme_color2,command=self.check)
	self.submit.place(x=975,y=450)
        ...
        self.menubar.add_command(label='Toggle Fullscreen',command=self.fullscreen)
    ...
    def resize(self, event):
        ...
        if(w > 1200):              #condition to filter required event
                x, y = (w-1366)/2, (h-768)*1/2
		self.grid.place(x+100,y+70)
		self.timer.place(x+950,y+265)
		self.submit.place(x=x+975,y=y+450)
		self.canv.coords("titleBanner",x+750,y+65)
	...
	self.full = not self.full
        self.root.wm_attributes('-fullscreen',self.full)
